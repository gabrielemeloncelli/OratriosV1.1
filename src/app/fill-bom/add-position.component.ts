        ...
        if (detail.displayInsertPosition)
        {
          ...
          if (this.hideGroup)
          {
            if (this.hidePart)
            {
              ...
              var partOption = new Option({value: '' + this.uiStatusService.commodityPart.id,
               label: '' + this.uiStatusService.commodityPart.id});
              this.partSelected(partOption);
            }
            else
            {
              var groupOption = new Option({value: '' + this.uiStatusService.commodityGroup.id,
        ...
        for (let attribute of attributes)
        {
          ...
          .subscribe(v => 
          {
            if (true && v && v.length > 0)
            {
              ...
              this.allowedValues[index] = v.map(v1 =>  new Option({value: v1.value,
                 label: v1.value}));
            }
  ...
  ngOnInit()
  {
    this.commodityGroupService.groups.subscribe(
      (groups: CommodityGroup[]) => {
        this.groups = groups.map(g => new Option({value: g.id.toString(),
    ...
    this.commodityPartService.parts.subscribe(
      (parts: CommodityPart[]) => {
        this.parts = parts.map(p => new Option({value: p.id.toString(),
